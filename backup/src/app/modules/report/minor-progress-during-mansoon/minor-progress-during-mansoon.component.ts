...
@Component({
  selector: 'app-minor-progress-during-mansoon',
  templateUrl: './minor-progress-during-mansoon.component.html',
  styleUrls: ['./minor-progress-during-mansoon.component.scss']
})
export class MinorProgressDuringMansoonComponent  implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dbCallingService.getMinorNallPreMonsoonProgressData(obj).subscribe(
      ...
      //    "Ward":"",
      //    "Workcode":"",
      //    "TotalSiltQuantity":this.listminor.reduce((p,c)=>p+Number(c.TotalDesiltQuantity),0).toFixed(2),
      //    "PremonsoonDesiltQuantity": this.listminor.reduce((p,c)=>p+Number(c.PremonsoonDesiltQuantity),0).toFixed(2) ,
      //    "DuringMonsoonDesiltQuantity": this.listminor.reduce((p,c)=>p+Number(c.DuringMonsoonDesiltQuantity),0).toFixed(2),
      //    "TotalVehicle": this.listminor.reduce((p,c)=>p+Number(c.TotalVehicle),0).toFixed(2),
      ...
      let eachRec = {
       ...
       this.listminor=this.lstMinorResults.Data;
       console.log("result",this.listminor);
    ...
    getAGGridReady() {
      ...
      this.columnDefs = [
        ...
        { headerName: 'Pre monsoon Quanitity', field: 'TotalPremonsoonDesiltQuantity', },
        { headerName: 'During Monsoon Quanitity', field: 'TotalAftermonsoonDesiltQuantity',},
    ...
    download() {
      const fileName = "MinorNallaDuringMonsoonProgressReport_"+ moment(new Date()).format('DDMMYYYY')
      const recordList = [];
      this.listminor.forEach(rec => {
        let eachRec = {
          ...
          "Pre monsoon Quanitity": rec.TotalPremonsoonDesiltQuantity,
          "During Monsoon Quanitity": rec.TotalAftermonsoonDesiltQuantity,
        ...
        content: [
          {
            table: {
              ...
              body: [
                ...
                ['', { text: 'During-Monsoom Progress report of De-silting', alignment: "center" }, ''],
