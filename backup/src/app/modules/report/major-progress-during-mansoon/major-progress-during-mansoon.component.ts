...
@Component({
  selector: 'app-major-progress-during-mansoon',
  templateUrl: './major-progress-during-mansoon.component.html',
  styleUrls: ['./major-progress-during-mansoon.component.scss']
})
export class MajorProgressDuringMansoonComponent {
  ...
  rowSelection;
  frameworkComponents;
  fileName = 'MajorNallahDuringMonsoonProgressReport.xlsx';
  ...
  getAGGridReady() {      
    ...
    this.columnDefs = [
      ...
      { headerName: 'Nallah No', field: 'NallahNo',  maxWidth:80 },    
      { headerName: 'Catchment Number', field: 'CatchmentNumber',  maxWidth:90},
      { headerName: 'Nallah System', field: 'NallahSystem',  maxWidth:200},
      { headerName: 'Nallah Length', field: 'NallahLength',  maxWidth:80},
  ...
  download() {
    const fileName = "Major_Nalla_Pre-Monsoon_Progress";
    const recordList = [];
    this.lstReportData.forEach(rec => {
      let eachRec = {
        "Nalla ID": rec.SiltQuantityID,
        "Zone": rec.Zone,
        "Ward": rec.Ward,
        "Executive Engg":rec.ExecutiveEngg,
