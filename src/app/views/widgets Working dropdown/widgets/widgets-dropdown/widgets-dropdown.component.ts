...
export class WidgetsDropdownComponent implements OnInit, AfterContentInit {
  public yesterDate: Date;
  constructor(
    private changeDetectorRef: ChangeDetectorRef, private dbCallingService: DbCallingService
  ) { }
  wbWidgetData: any[] = [];
  data: any[] = [];
  options: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.yesterDate = new Date(moment(theDate).subtract(1, 'day').format('YYYY-MM-DD'))
    this.dbCallingService.getWBWidgetData().subscribe(
      (result) => {
        this.wbWidgetData = result.WBWidgetTable;
        this.ttlWT = this.wbWidgetData.reduce((a, b) => a + b.ActNetWt, 0).toFixed(2)
        this.ttlCount = this.wbWidgetData.reduce((a, b) => a + b.VehicleCount, 0)
      },
      (err) => {
        console.log(err)
      }
